...
def gjenero_permbledhje_llm(konteksti, agjenti):
    ...
    if not provider or not api_key:
        return (
            "LLM nuk eshte konfiguruar. Shto LLM_PROVIDER dhe API key te secrets.toml."
        )
...
def llogarit_rfm(df, kolona_klienti="klienti"):
    ...
    try:
        rfm["R_Skor"] = (
            pd.qcut(
                rfm["R"], q=min(5, rfm["R"].nunique()), labels=False, duplicates="drop"
            )
            + 1
        )
...
def _ndertoj_full_map_agjent(df, df_agj, agj_sel, start_date, end_date, rritja, sot):
    ...
    konflikti_global = (
        df[mask_ref].groupby("klienti")["forcashitese"].nunique().reset_index()
    )
    konflikti_global["ka_konflikt"] = konflikti_global["forcashitese"] > 1
    full_map = full_map.merge(
        konflikti_global[["klienti", "ka_konflikt"]], on="klienti", how="left"
    )
...
def _render_per_agjent(
    ...
    full_map = _ndertoj_full_map_agjent(
        df, df_agj, agj_sel, start_date, end_date, rritja, sot
    )
    ...
    if not kandidatet_geo.empty:
        ...
        st.success(
            "Plani: {} kliente, {:,.0f} kg, {:.1f} km".format(
    ...
    for _, rresht in rendor_iter:
        ...
        if pd.isna(segm):
            segm = ""
        konfl_str = "Konflikt" if rresht.get("ka_konflikt", False) else ""
    ...
    if col_e3.button("Permbledhje AI", use_container_width=True):
        with st.spinner("Po therras LLM..."):
            kontekst = {
                ...
                "konflikte": int((df_plan["Konflikt"] == "Konflikt").sum()),
...
# =========================================================
def _render_globale(
    df, df_klientet_regjistri, start_date, end_date, rritja, maks_vizita, data_plani
):
    sot = pd.Timestamp(data_plani)
    agjentet = sorted(
        [a for a in df["forcashitese"].dropna().astype(str).unique() if a]
    )
    if not agjentet:
    ...
    for i, agj in enumerate(agjentet):
        ...
        if df_agj.empty:
            continue
        try:
            fm = _ndertoj_full_map_agjent(
                df, df_agj, agj, start_date, end_date, rritja, sot
            )
        ...
        permbledhja_agj.append(
            {
                ...
                "Priority_Mes": (
                    round(float(top_k["Priority_Score"].mean()), 0)
                    if not top_k.empty
                    else 0
                ),
            }
    ...
    progress.empty()
    df_sum = pd.DataFrame(permbledhja_agj).sort_values(
        "Target_KG_Mbetjes", ascending=False
    )
    ...
    m2.metric(
        "Total Klientet ne Plane", "{:,.0f}".format(df_sum["Klientet_ne_Plan"].sum())
    )
...
def render_plan_ditor(
    ...
    if (
        grup_sel
        and str(grup_sel).strip().lower() not in ("te gjitha", "të gjitha")
        and "grup_filtri" in df.columns
    ):
