# =========================================================
# AXION - Modulet AI (v3.4 - me buton manual per Te gjithe)
# =========================================================
# Eksporton: render_plan_ditor
# Mode "Te gjithe": kerkon klikim butoni per te gjeneruar raportin.
# =========================================================
...
def gjenero_permbledhje_llm(konteksti, agjenti):
    ...
    prompt_sistemi = (
        "Je nje konsulent shitjesh ne Shqiperi. Pergjigju ne shqip. "
        "Jep keshilla konkrete. Strukturo si: (1) Diagnoza, (2) 3 Prioritetet, "
        "(3) Klientet/Agjentet kyc. Maks. 250 fjale."
    )
    prompt_user = (
        "Te dhenat per " + str(agjenti) + ":\n\n"
    ...
    try:
        ...
        return "Paketa mungon: pip install " + paketa + "\n\n" + str(e)
    except Exception as e:
        return "Gabim LLM: " + str(e)
...
def _ndertoj_full_map(df, df_agj, agj_sel, start_date, end_date, rritja, sot):
    ...
    klient_agj = (
        ...
        .reset_index()
        .rename(columns={"forcashitese": "lista_agj"})
    )
    klient_agj["nr_agj"] = klient_agj["lista_agj"].apply(len)
    klient_agj["ka_konflikt"] = klient_agj["nr_agj"] > 1
    klient_agj["agj_te_tjere"] = klient_agj["lista_agj"].apply(
        lambda lst: [a for a in lst if a != agj_sel]
    )
    klient_agj["konflikt_detaj"] = klient_agj["agj_te_tjere"].apply(
        lambda lst: ", ".join(lst) if lst else ""
    )

    full_map = full_map.merge(
        klient_agj[["klienti", "ka_konflikt", "konflikt_detaj", "nr_agj"]],
        on="klienti", how="left",
    )
    full_map["ka_konflikt"] = full_map["ka_konflikt"].fillna(False)
    full_map["konflikt_detaj"] = full_map["konflikt_detaj"].fillna("")
    full_map["nr_agj"] = full_map["nr_agj"].fillna(1).astype(int)
...
def _render_per_agjent(df, df_klientet_regjistri, agj_sel, start_date, end_date, rritja,
    ...
    full_map = _ndertoj_full_map(df, df_agj, agj_sel, start_date, end_date, rritja, sot)
    ...
    if not kandidatet_geo.empty:
        kolonat = ["klienti", "rajoni", "latitude", "longitude",
                   ...
                   "ka_konflikt", "konflikt_detaj", "nr_agj",
                   "Target_Muaj", "Ecuria"]
    ...
    for _, rresht in rendor_iter:
        ...
        ka_k = rresht.get("ka_konflikt", False)
        detaj = rresht.get("konflikt_detaj", "")
        if pd.isna(detaj):
            detaj = ""
        nr_a = _safe_int(rresht.get("nr_agj", 1), 1)
        if ka_k and detaj:
            konfl_str = "Me " + str(nr_a - 1) + " agjent: " + str(detaj)
        elif ka_k:
            konfl_str = "Po"
        else:
            konfl_str = ""
    ...
    csv_detaj = pd.DataFrame(detaj_list).to_csv(index=False).encode("utf-8-sig")
    ...
    if col_e3.button("Permbledhje AI", use_container_width=True):
        with st.spinner("Po therras LLM..."):
            ...
            perm = gjenero_permbledhje_llm(kontekst, agjenti=agj_sel)
...
def _render_globale(df, df_klientet_regjistri, start_date, end_date, rritja,
    ...
    for i, agj in enumerate(agjentet):
        ...
        if df_agj.empty:
            continue
        try:
            fm = _ndertoj_full_map(df, df_agj, agj, start_date, end_date, rritja, sot)
            fm = _shtoj_koordinatat(fm, df_klientet_regjistri)
        except Exception as e:
            st.caption("Gabim per " + agj + ": " + str(e))
            continue
    ...
    if df_sum.empty:
        st.warning("Asnje agjent nuk ka te dhena.")
        return
    ...
    if col_g2.button("Permbledhje AI (Globale)", use_container_width=True):
        with st.spinner("Po therras LLM..."):
            ...
            agj_me_konfl = df_sum[df_sum["Konflikte"] > 0].sort_values(
            ...
            kontekst_global = {
                ...
                "top_5_agjente_me_konflikte": agj_me_konfl,
            }
            perm_g = gjenero_permbledhje_llm(
...
def render_plan_ditor(df_raw, df_klientet_regjistri, agj_sel,
    ...
    titulli = "Plani Ditor"
    if agj_sel and not _eshte_te_gjithe(agj_sel):
        titulli += " - " + str(agj_sel)
    st.title(titulli)

    info_pjeset = []
    if grup_sel and not _eshte_te_gjitha(grup_sel):
    ...
    if agj_sel and _eshte_te_gjithe(agj_sel):
        info_pjeset.append("Mode: Raport global (manual)")
    if info_pjeset:
    ...
    if grup_sel and not _eshte_te_gjitha(grup_sel) and "grup_filtri" in df.columns:
    ...
    if _eshte_te_gjithe(agj_sel):
        # Mos auto-gjenero - kerko klikim butoni sepse llogaritja eshte e ngadalte
        if "show_global_report" not in st.session_state:
            st.session_state["show_global_report"] = False

        st.warning(
            "Zgjidh nje agjent specifik ne sidebar per nje plan te shpejte, "
            "ose klikoni butonin me poshte per te gjeneruar raportin global "
            "(zgjat 10-30 sekonda)."
        )

        col_btn1, col_btn2 = st.columns(2)
        if col_btn1.button("Gjenero Raportin Global", use_container_width=True, type="primary"):
            st.session_state["show_global_report"] = True
        if col_btn2.button("Fshih Raportin", use_container_width=True):
            st.session_state["show_global_report"] = False

        if st.session_state["show_global_report"]:
            st.divider()
            _render_globale(df, df_klientet_regjistri, start_date, end_date, rritja,
                            maks_vizita, data_plani)
        else:
            st.info("Zgjidh agjent ne sidebar ose klikoni 'Gjenero Raportin Global'.")
    else:
        if "show_global_report" in st.session_state:
            st.session_state["show_global_report"] = False
