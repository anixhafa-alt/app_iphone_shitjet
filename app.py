...
def load_data():
    ...
    try:
        ...
        # Pastrimi i kolonave nga hapësirat e tepërta (Fix për Bekimin dhe të tjerët)
        df.columns = df.columns.str.strip().str.replace('"', '')
        df['ForcaShitese'] = df['ForcaShitese'].astype(str).str.strip()
        df['Klienti'] = df['Klienti'].astype(str).str.strip()
        df['Artikulli'] = df['Artikulli'].astype(str).str.strip()
...
if df is not None:
    ...
    # Filtri i Klientit - Kërkojmë në të gjithë DF për siguri
    ...
    st.caption("ℹ️ Çmimet bazohen në shitjen e fundit. Formatimi me ndarje mijëshe (1,000) dhe 1 shifër pas presjes te çmimi.")
    ...
    c2.metric("Cmimi Mesatar Planit", f"{c_m:,.1f} L/kg")
