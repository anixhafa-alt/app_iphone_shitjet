...
def load_data():
    ...
    try:
        ...
        # Trajtimi i kolonës 'kat' për të shmangur gabimin 'float object has no attribute upper'
        ...
        # Ruajmë vlerën historike për llogaritje mesatareje
...
if df is not None:
    # Gjejmë datën më të fundit në të gjithë bazën
    data_fundit_db = df['Data'].max().strftime('%d/%m/%Y')

    # --- LOGJIKA E ÇMIMIT TË FUNDIT (Nga fillimi i kohës deri sot) ---
    ...
    # 1. Filtri i Periudhës Referente
    ...
    st.info(f"📅 Update i fundit në DB: **{data_fundit_db}** | Grupi: **{grup_sel}**")
    ...
    def generate_html_report(dataframe):
        ...
        html = f"<html><head><style>body{{font-family:sans-serif;}} table{{width:100%; border-collapse:collapse; margin-bottom:20px;}} th,td{{border:1px solid #ddd; padding:8px; text-align:left;}} th{{background-color:#f2f2f2;}} .num{{text-align:right;}}</style></head><body>"
