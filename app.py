...
if df is not None:
    ...
    # --- METRICS ---
    st.title(f"🎯 Plani: {grup_sel}")
    st.info(f"📅 Update: **{data_fundit_db}**")
    ...
    # --- LOGJIKA E EKSPORTIT ME TOTAL ---
    def generate_html_report(dataframe):
        report_dt = datetime.now().strftime('%d/%m/%Y %H:%M')
        html = f"<html><head><style>body{{font-family:sans-serif;}} table{{width:100%; border-collapse:collapse; margin-bottom:40px;}} th,td{{border:1px solid #ddd; padding:8px; text-align:left;}} th{{background-color:#f2f2f2;}} .num{{text-align:right;}} .total-row{{font-weight:bold; background-color:#eef2f7;}}</style></head><body>"
        html += f"<h1>Raporti i Planit - {report_dt}</h1>"
        for agjent in sorted(dataframe['ForcaShitese'].unique()):
            ...
            # Llogarisim totalet per agjentin
            total_kg_agj = kat_df['Plani_KG'].sum()
            total_vlera_agj = kat_df['Vlera_Planifikuar'].sum()
            mesatarja_agj = total_vlera_agj / total_kg_agj if total_kg_agj > 0 else 0
            ...
            # Shtojme rreshtin e TOTALIT per agjentin
            html += f"<tr class='total-row'><td>TOTALI {agjent}</td><td class='num'>{mesatarja_agj:,.1f} L</td><td class='num'>{total_kg_agj:,.0f}</td></tr>"
            html += "</tbody></table>"
        html += "</body></html>"
        return html
