...
def load_data_combined():
    try:
        ...
        # C. Bashkimi (Merge)
        df = pd.merge(
            df_sql, df_mapping, left_on="Artikulli", right_on="EMERTIMI", how="left"
        )
...
if df is not None:
    ...
    # --- AGREGIMI ---
    gp = (
        dff.groupby(["ForcaShitese", "Klienti", "kat", "Artikulli"])
    ...
    def generate_html_report(dataframe):
        ...
        for agjent in sorted(dataframe["ForcaShitese"].unique()):
            ...
            kat_df["Cmimi"] = kat_df["Vlera_Planifikuar"] / kat_df["Plani_KG"].replace(
                0, 1
            )
