...
def load_data_combined():
    try:
        ...
        df_mapping["KG/SKU"] = pd.to_numeric(
            df_mapping["KG/SKU"], errors="coerce"
        ).fillna(0)

        # Merge dhe Llogaritja e KG
        df = pd.merge(
            df_sql, df_mapping, left_on="KodiArt", right_on="KODI", how="left"
        )
        df["kg"] = df["Sasia"] * df["KG/SKU"]
        df.rename(columns={"KATEG": "kat"}, inplace=True)
        df["kat"] = df["kat"].fillna("ETJ")
        df["Vlera_Historike"] = (
            pd.to_numeric(df["VleraRresht"], errors="coerce")
            .fillna(0)
            .astype("float32")
        )
...
if df is not None and not df.empty:
    ...
    last_prices = df.sort_values("Data").drop_duplicates("KodiArt", keep="last")[
    ...
    agj_list = sorted(
        [str(x) for x in df["ForcaShitese"].unique() if x not in ["nan", "None"]]
    )
    ...
    max_dt = (
        df["Data"].max().strftime("%d/%m/%Y") if pd.notnull(df["Data"].max()) else "N/A"
    )
    st.info(
        f"📅 Burimi: **SQL Server** | Update i fundit: **{max_dt}** | Periudha: **{n_months} muaj**"
    )
    ...
    with col_left:
        fig_kat = px.pie(
            ...
            names="kat",
            hole=0.4,
            title="Shpërndarja sipas Kategorive (KG)",
        )
    ...
    with col_right:
        ...
        fig_agj = px.pie(
            ...
            names="ForcaShitese",
            hole=0.4,
            title="Shpërndarja sipas Agjentëve (KG)",
        )
