...
def load_data_combined():
    try:
        ...
        # B. Leximi i Excel - Specifikojmë sheet_name='produktet'
        # Këtu u bë ndryshimi për të gjetur kolonat korrektet
...
if df is not None and not df.empty:
    ...
    # --- SHFAQJA ---
    st.title("🚀 Plani Real-Time (SQL + Sheet: produktet)")
    max_date = df['Data'].max()
    last_update_str = max_date.strftime('%d/%m/%Y') if pd.notnull(max_date) else "Pa datë"
    st.info(f"📅 Update: **{last_update_str}** | Llogaritja: **Sasia * KG/SKU**")
