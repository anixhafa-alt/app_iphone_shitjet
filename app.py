...
# --- NGARKIMI I TE DHENAVE (SQL + EXCEL MAPPING + KONVERTIMI KG) ---
@st.cache_data(ttl=3600)
def load_data_combined():
    try:
        # A. Lidhja me SQL (Marrim Sasia ne vend te kg)
        conn = st.connection("sql", type="sql")
        sql_query = "SELECT Data, ForcaShitese, Klienti, Artikulli, Sasia, VleraRresht FROM dbo.GetRaportiMadhView"
        ...
        df_sql['Sasia'] = pd.to_numeric(df_sql['Sasia'], errors='coerce').fillna(0)
        df_sql = df_sql.dropna(subset=['Data'])

        # C. Merr kategoritë dhe peshat nga Excel-i 'produkte+.xlsx'
        # Artikulli=EMERTIMI, kat=KATEG, Pesha=KG/SKU
        ...
        # E. LLOGARITJA E KG (Sasia copë * Pesha për njësi)
...
if df is not None and not df.empty:
    # Llogaritja e çmimit të fundit (Vlera / kg)
    ...
    # --- TITULLI DHE INFO ---
    st.title("🚀 Plani Real-Time (Konvertim KG aktiv)")
    max_date = df['Data'].max()
    last_update_str = max_date.strftime('%d/%m/%Y') if pd.notnull(max_date) else "Pa datë"
    st.info(f"📅 Update: **{last_update_str}** | Metoda: **Sasia (SQL) * KG/SKU (Excel)**")

    # --- METRICS & TABS ---
    t_kg_p = gp['Plani_KG'].sum()
    t_v_p = gp['Vlera_Planifikuar'].sum()
    c1, c2 = st.columns(2)
    c1.metric("Plani KG Totale", f"{t_kg_p:,.0f}")
    c2.metric("Vlera Totale Plani", f"{t_v_p:,.0f} L")

    st.divider()
    t1, t2 = st.tabs(["📊 Kategoritë", "👤 Agjentët"])
    config_kol = {"Plani_KG": st.column_config.NumberColumn("KG Plan", format="%d"), "Vlera_Planifikuar": st.column_config.NumberColumn("Vlera Planit", format="%d")}
    ...
    def generate_html_report(dataframe):
        ...
        for agjent in sorted(dataframe['ForcaShitese'].unique()):
            ...
            html += f"<tr class='total-row'><td>TOTALI {agjent}</td><td class='num'>{kat_df['Plani_KG'].sum():,.0f}</td><td class='num'>{kat_df['Vlera_Planifikuar'].sum():,.0f} L</td></tr>"
            html += "</tbody></table>"
        html += "</body></html>"
        return html
