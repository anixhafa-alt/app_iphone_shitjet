...
def load_data():
    ...
    try:
        ...
        # --- KRIJIMI I FILTRIT TE RI (OLIM, ETJ, DEKA) ---
...
if df is not None:
    ...
    # 1. Filtri i ri i Grupeve (OLIM, ETJ, DEKA)
    ...
    st.info(f"📅 Update i fundit: **{data_fundit}** | Filtri aktiv: **{grup_sel}**")
    ...
    c3.metric("Çmimi Fundit Mes.", f"{cm_mes_plan:,.1f} L/kg")
    c4.metric("Nr. Artikujve", len(gp))
    ...
    if klientet_selected:
        ...
        with t1:
            ...
            st.dataframe(df_k[['kat', 'Cmimi_Mes_Grup', 'Plani_KG', 'Vlera_Planifikuar']].sort_values('Plani_KG', ascending=False), 
                         width='stretch', hide_index=True, column_config=config_kolonave)
        with t2:
            ...
            st.dataframe(df_a[['ForcaShitese', 'Cmimi_Mes_Grup', 'Plani_KG', 'Vlera_Planifikuar']].sort_values('Plani_KG', ascending=False), 
                         width='stretch', hide_index=True, column_config=config_kolonave)
        with t3:
            ...
            st.dataframe(df_kl[['Klienti', 'ForcaShitese', 'Cmimi_Mes_Grup', 'Plani_KG', 'Vlera_Planifikuar']].sort_values('Plani_KG', ascending=False), 
                         width='stretch', hide_index=True, column_config=config_kolonave)
